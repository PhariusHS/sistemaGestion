...
export const createPlaca = async (req, res) => {
  try {
    const { color, ancho, largo, alto, densidad, masa, date, estado } =
      req.body;
...
export const updatePlaca = async (req, res) => {
  try {
    const { color, ancho, largo, alto, densidad, masa, estado, date } =
      req.body;
    const PlacaUpdated = await Placa.findOneAndUpdate(
      { _id: req.params.id },
      { color, ancho, largo, alto, densidad, masa, estado, date },
      { new: true }
    );
